...
from .calculator   import NaturalGasLevels
...
# Strategy Branding
STRATEGY_NAME = "NATURALGAS • MathZing"
...
def load_today_states():
    ...
    if missing:
        ...
        logger.info("Natural Gas Strategy: today's states loaded/synced from DB")
...
def sync_live(instrument: str, type: str, sim: dict):
    ...
    logger.info(f"Natural Gas Strategy: {instrument} {type} synced from backtest to live")
    return {"success": True}
...
def _recalculate_active_levels(state: dict):
    ...
    if state.get("long_state") in ("ACTIVE_P1", "ACTIVE_P2"):
        ...
        if ep:
            lvl["t_l"] = rt(ep * 1.04)
            if "sl1_long" in lvl:
                lvl["sl1_long"]["a"] = rt(ep * 0.96)
                lvl["sl1_long"]["sl"] = max(lvl["sl1_long"]["a"], lvl["sl1_long"].get("b", 0))
            if "sl2_long" in lvl:
                lvl["sl2_long"]["a"] = rt(ep * 0.96)  # Phase 2 SL: entry - 4% (NOT cost itself)
    ...
    if state.get("short_state") in ("ACTIVE_P1", "ACTIVE_P2"):
        ...
        if ep:
            lvl["t_s"] = rt(ep * 0.96)
            if "sl1_short" in lvl:
                lvl["sl1_short"]["a"] = rt(ep * 1.04)
                lvl["sl1_short"]["sl"] = min(lvl["sl1_short"]["a"], lvl["sl1_short"].get("b", 9999999))
            if "sl2_short" in lvl:
                lvl["sl2_short"]["a"] = rt(ep * 1.04)  # Phase 2 SL: entry + 4% (NOT cost itself)
                lvl["sl2_short"]["sl"] = min(lvl["sl2_short"]["a"], lvl["sl2_short"].get("b", 9999999))


def set_levels_from_natural_gas_levels(inst: str, gl: NaturalGasLevels):
    ...
    if saved_levels_json:
        try:
            ...
            logger.info(f"Natural Gas Strategy: Applied saved default levels for {inst}")
        except Exception as e:
    ...
    with _lock:
        ...
        _live[inst].update({
            ...
            "levels":           d,
            "long_state":       _live[inst].get("long_state", "PENDING"),
            "long_entry_price": _live[inst].get("long_entry_price"),
            "long_entry_date":  _live[inst].get("long_entry_date"),
            "long_lot1_closed": _live[inst].get("long_lot1_closed", False),
            "long_exit_price":  _live[inst].get("long_exit_price"),
            "long_exit_reason": _live[inst].get("long_exit_reason"),
            "long_pnl":         _live[inst].get("long_pnl", 0),
            "short_state":      _live[inst].get("short_state", "PENDING"),
            "short_entry_price":_live[inst].get("short_entry_price"),
            "short_entry_date": _live[inst].get("short_entry_date"),
            "short_lot1_closed":_live[inst].get("short_lot1_closed", False),
            "short_exit_price": _live[inst].get("short_exit_price"),
            "short_exit_reason":_live[inst].get("short_exit_reason"),
            "short_pnl":        _live[inst].get("short_pnl", 0),
            "auto_trade":       saved_auto,
            "ltp":              gl.raw_days[0].get("close") if gl.raw_days else None,
            "last_update":      _live[inst].get("last_update"),
        })
    ...
    for inst in INSTRUMENTS:
        ...
        if "09:00:00" <= ts < "09:10:00":
            ...
            if long_st == "ACTIVE_P1" and t_l and ltp >= t_l:
                ...
                mult = MULTIPLIERS.get(inst, 1250)
...
def _handle_915_sl_reset(inst: str, state: dict, ltp: float):
    """
    Rule: At 9:15 AM, if price has gapped beyond SL, reset SL instead of exiting.
    Buffer: 0.4% for Natural Gas.
    """
    ...
    try:
        ...
        if state["long_state"] == "GAP":
            ...
            changed = True
            logger.info(f"{inst}: GAP RECOVERY (LONG) at 9:15 — Gap E_L={new_e_l} | Gap T_L={new_t_l} | SL1={new_sl1_l} | Dashboard E_L unchanged={lvl.get('e_l')}")
            tg.send_msg(
                f"📊 *{inst} GAP RECOVERY at 9:15 AM*\n15-min High: *{m15_high}*\n"
                f"Gap Long Entry: *{new_e_l}* (High × 1.004)\nTarget: *{new_t_l}* | SL1: *{new_sl1_l}*\n"
                f"Status → PENDING"
            )
        ...
        if state["short_state"] == "GAP":
            ...
            changed = True
            logger.info(f"{inst}: GAP RECOVERY (SHORT) at 9:15 — Gap E_S={new_e_s} | Gap T_S={new_t_s} | SL1={new_sl1_s} | Dashboard E_S unchanged={lvl.get('e_s')}")
            tg.send_msg(
                f"📊 *{inst} GAP RECOVERY at 9:15 AM*\n15-min Low: *{m15_low}*\n"
                f"Gap Short Entry: *{new_e_s}* (Low × 0.996)\nTarget: *{new_t_s}* | SL1: *{new_sl1_s}*\n"
                f"Status → PENDING"
            )
        ...
        if state["long_state"] in ("ACTIVE_P1", "ACTIVE_P2"):
            ...
            curr_sl = lvl.get(sl_key, {}).get("sl", 0)
            if curr_sl and ltp <= curr_sl:
                new_sl = rt(m15_low * 0.996)
                lvl[sl_key]["sl"] = new_sl
                lvl[sl_key]["b"] = new_sl
                changed = True
                logger.info(f"{inst}: LONG SL Gap Reset at 9:15. New SL: {new_sl} (based on 15m low {m15_low})")
                tg.send_msg(f"🔔 *{inst} SL RESET (9:15 AM)*\nLong SL was breached by gap. New Trailing SL set at *{new_sl}* (Low - 0.4%)")
...
def _send_to_main_app(inst: str, action: str, price: float, sl: float,
                      targets: list, state: dict, auto: bool, strategy_type: str = "NaturalGas"):
...
def start_monitor():
    ...
    def _loop():
        logger.info("Natural Gas Strategy monitor started")
        while True:
            try:
                _monitor_tick()
            except Exception as e:
