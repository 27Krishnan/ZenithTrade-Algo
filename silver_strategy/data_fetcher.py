...
INSTRUMENTS = {
    "SILVER":    {"exchange": "MCX", "search": "SILVER",    "lots": 2},
    "SILVERM":   {"exchange": "MCX", "search": "SILVERM",   "lots": 2},
    "SILVERMIC": {"exchange": "MCX", "search": "SILVERMIC", "lots": 2},
}
...
def _find_near_month_token(name: str, as_of_date: datetime | None = None) -> dict | None:
    ...
    for row in data:
        ...
        if name == "SILVER":
            if not re.match(r'^SILVER\d', sym): continue
        elif name == "SILVERM":
            if not re.match(r'^SILVERM\d', sym): continue
        elif name == "SILVERMIC":
            if not re.match(r'^SILVERMIC\d', sym): continue
        else:
            continue
    ...
    logger.info(f"Resolved {name} CURRENT : {chosen_current['trading_symbol']} (token={chosen_current['token']})")
    if chosen_next:
        logger.info(f"Resolved {name} NEXT    : {chosen_next['trading_symbol']} (token={chosen_next['token']})")
