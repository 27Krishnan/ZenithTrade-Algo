...
Given an instrument and a START date (the day we would have ENTERED the trade):
  ...
  6. Continue with Lot-2 only using ROLLING SL2 that updates daily:
     - Each new completed day shifts the 4-day window
     - New L4/H4 → new SL2 via max/min(Point_A, Point_B)
...
def _snapshot_levels(gl: SilverLevels, e_l, e_s, t_l, t_s, sl1_l, sl1_s, sl2_l, sl2_s) -> dict:
    levels = gl.to_dict()
    levels["e_l"] = e_l
    levels["e_s"] = e_s
    levels["t_l"] = t_l
...
def _recalc_sl2(gl: SilverLevels, history: list[dict], direction: str, actual_entry: float) -> tuple[float, str]:
    """
    ...
    dates_used = " / ".join(d["date"][5:] for d in four)   # MM-DD format

    if direction == "long":
        pt_a = rt(actual_entry * 0.98)
        pt_b = rt(l4 * 0.9988)
        new_sl = max(pt_a, pt_b)
        return new_sl, f"Window [{dates_used}] | L4={l4:.2f} | A={pt_a:.2f} B={pt_b:.2f} -> SL2={new_sl:.2f}"
    else:
        pt_a = rt(actual_entry * 1.02)
        pt_b = rt(h4 * 1.0012)
...
def _simulate_multiday(
    ...
    short_lot1_pnl = None
    short_pnl = None
    mult = MULTIPLIERS.get(gl.instrument, 1)
    events = []
        ...
        for c in intraday:
            ...
            if day_num == 0 and t == recovery_t:
                ...
                if long_state == "GAP":
                    ...
                    pt_a_l1 = rt(new_e_l * 0.98); sl1_l = max(pt_a_l1, gl.sl1_long_b)
                    pt_a_l2 = rt(new_e_l * 0.98); sl2_l = max(pt_a_l2, gl.sl2_long_b)
                    e_l = new_e_l; t_l = new_t_l
                ...
                if short_state == "GAP":
                    ...
                    pt_a_s1 = rt(new_e_s * 1.02); sl1_s = min(pt_a_s1, gl.sl1_short_b)
                    pt_a_s2 = rt(new_e_s * 1.02); sl2_s = min(pt_a_s2, gl.sl2_short_b)
                    e_s = new_e_s; t_s = new_t_s
