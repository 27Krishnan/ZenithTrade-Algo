...
from .monitor       import set_levels_from_gold_levels, get_live_state, _set_state
...
def _fetch_and_broadcast(broadcast=None):
    ...
    logger.info(f"Gold Strategy: Data fetch starting (requested_broadcast={broadcast})...")
    levels_map = {}
    ...
    for inst in INSTRUMENTS:
        try:
            ...
            set_levels_from_gold_levels(inst, gl)
            ...
            if l_st in ("ACTIVE_P1", "ACTIVE_P2") or s_st in ("ACTIVE_P1", "ACTIVE_P2"):
                # Use the live levels (post-override) for accurate SL2 Trail
    ...
    if broadcast is True:
        ...
        if now.hour == 8 and now.minute >= 25:
             should_broadcast = True
             logger.info(f"Gold Strategy: Broadcasting morning briefing (Time: {now.strftime('%H:%M')})")
    
    if should_broadcast:
        tg.send_morning_alert(levels_map, live_states=live_states)
        # Ensure we record that it was sent today if it was a manual/cron True broadcast
        now = datetime.now(IST)
...
def _reverify():
    ...
    logger.info("Gold Strategy: 8:50 AM re-verification...")
    for inst in INSTRUMENTS:
        try:
...
def _place_target_orders():
    """Simple market open check at 9:00 AM / 5:00 PM."""
    logger.info("Gold Strategy: market check sequence...")
    from .data_fetcher import get_ltp
    # Check GOLD LTP as a proxy for market status
    state = get_live_state("GOLD")
    if state and state.get("token"):
        ltp = get_ltp(state["token"], state["trading_symbol"])
        is_open = (ltp is not None)
        tg.send_market_open(is_open)
    else:
        tg.send_market_open(False)
...
def _place_entry_and_sl_orders():
    ...
    """
    logger.info("Gold Strategy: placing entry/SL orders...")
    ...
    for inst in INSTRUMENTS:
        ...
        if l_st == "PENDING":
            ...
            if e_l and sl1l:
                ...
                # tg.send removed to reduce noise
...
def _handle_gap_recovery():
    ...
    """
    logger.info("Gold Strategy: checking for gap recovery...")
    from .data_fetcher import angel_api
    ...
    for inst in INSTRUMENTS:
        ...
        if ls == "GAP" or ss == "GAP":
            ...
            if ls == "GAP":
                ...
                pt_a_l1 = rt(new_e_l * 0.985); pt_b_l1 = lvl["sl1_long"]["b"]; sl1_l = max(pt_a_l1, pt_b_l1)
                pt_a_l2 = rt(new_e_l * 0.985); pt_b_l2 = lvl["sl2_long"]["b"]; sl2_l = max(pt_a_l2, pt_b_l2)
            ...
            if ss == "GAP":
                ...
                pt_a_s1 = rt(new_e_s * 1.015); pt_b_s1 = lvl["sl1_short"]["b"]; sl1_s = min(pt_a_s1, pt_b_s1)
                pt_a_s2 = rt(new_e_s * 1.015); pt_b_s2 = lvl["sl2_short"]["b"]; sl2_s = min(pt_a_s2, pt_b_s2)
...
def _update_trailing_sl2(inst: str, state: dict, direction: str, auto: bool):
    ...
    try:
        data = fetch_instrument_data(inst)
        if not data or len(data["candles"]) < 4:
            return
        ...
        if direction == "long":
            ...
            pt_a = rt(e_l * 0.985); pt_b = rt(new_l4 * 0.9988); new_sl = max(pt_a, pt_b)
            ...
            # Use stored 'a' (theoretical e_s*1.015) — do NOT override with actual entry
            stored_a = lvl.get("sl2_short", {}).get("a") or rt(e_s * 1.015)
...
def _update_trailing_sl1(inst: str, state: dict, direction: str, auto: bool):
    ...
    try:
        ...
        if direction == "long":
            e_l = state.get("long_entry_price") or lvl.get("e_l", 0)
            pt_a = rt(e_l * 0.985); pt_b = rt(l2 * 0.9988); new_sl = max(pt_a, pt_b)
            ...
            e_s = state.get("short_entry_price") or lvl.get("e_s", 0)
            pt_a = rt(e_s * 1.015); pt_b = rt(h2 * 1.0012); new_sl = min(pt_a, pt_b)
            if new_sl < current_sl1:
...
def start_scheduler():
    ...
    sched.add_job(auto_pull_latest, "cron", day_of_week="mon-fri", hour=8, minute=0, args=["Gold"], id="auto_pull")
    ...
    sched.start()
    logger.info("Gold Strategy scheduler started (Morning 9:00 | Evening 17:00 support)")
    return sched
