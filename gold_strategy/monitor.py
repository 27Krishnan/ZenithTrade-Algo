...
def load_today_states():
    ...
    for inst in INSTRUMENTS:
        ...
        if today_row:
            ...
            if needs_refresh:
                logger.info(f"Gold Strategy: Refreshing lookback levels for {inst} on {today}")
                missing = True
        else:
            ...
            if row:
                ...
                logger.info(f"Gold Strategy: Carrying forward active trade for {inst} from {row.date} to {today}")
    ...
    if missing:
        logger.info("Gold Strategy: No states found for today. Triggering automatic fetch...")
        ...
        logger.info("Gold Strategy: today's states loaded/synced from DB")
...
def sync_live(instrument: str, type: str, sim: dict):
    ...
    logger.info(f"Gold Strategy: {instrument} {type} synced from backtest to live")
    return {"success": True}
...
def _recalculate_active_levels(state: dict):
    ...
    if short_st in ("ACTIVE_P1", "ACTIVE_P2"):
        ...
        if ep:
            lvl["t_s"] = rt(ep * 0.985)
            if "sl1_short" in lvl:
                # SL1: tie 'a' to ACTUAL entry (protects Lot-1 from >1.5% loss)
...
def set_levels_from_gold_levels(inst: str, gl: GoldLevels):
    ...
    if saved_levels_json:
        try:
            ...
            logger.info(f"Gold Strategy: Applied saved default levels for {inst}")
        except Exception as e:
    ...
    if not row:
        with _lock:
            if inst in _live:
                logger.info(f"Gold Strategy: Fresh day initialization for {inst}. Resetting in-memory states to PENDING.")
                _live[inst].update({
                    "long_state": "PENDING",
    ...
    with _lock:
        ...
        if long_active or short_active:
            ...
            logger.info(f"{inst}: Trade ACTIVE — H4/L4 locked ({locked_h4}/{locked_l4}), "
                        f"H2 trail updated to {fresh_h2}")
            # Update the trailing SL 'b' component with fresh 2-day data
        ...
        # Final assignment: Use locked values if active, else use fresh levels
...
def _monitor_tick():
    ...
    for inst in INSTRUMENTS:
        ...
        if long_st == "PENDING" and e_l and ltp >= e_l:
            ...
            _recalculate_active_levels(_live[inst]) # Recalculate based on entry price
            # Refresh local variables for signaling
...
def _send_to_main_app(inst: str, action: str, price: float, sl: float,
                      targets: list, state: dict, auto: bool, strategy_type: str = "Gold"):
...
def start_monitor():
    ...
    def _loop():
        logger.info("Gold Strategy monitor started")
        while True:
            try:
                _monitor_tick()
            except Exception as e:
