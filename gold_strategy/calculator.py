...
class GoldLevels:
    ...
    def _calc(self):
        ...
        # Lot-1 Targets
        self.t_l = rt(self.e_l * 1.015)
        self.t_s = rt(self.e_s * 0.985)

        # Phase 1 SL — Long
        self.sl1_long_a = rt(self.e_l * 0.985)
        self.sl1_long_b = rt(self.l2 * 0.9988)
        self.sl1_long   = max(self.sl1_long_a, self.sl1_long_b)

        # Phase 1 SL — Short
        self.sl1_short_a = rt(self.e_s * 1.015)
        ...
        self.sl2_long_a = rt(self.e_l * 0.985)       # fixed
        ...
        self.sl2_short_a = rt(self.e_s * 1.015)      # fixed
    ...
    def update_from_actual_entry(self, actual_entry: float, side: str):
        ...
        if side == "long":
            ...
            self.t_l = rt(actual_entry * 1.015)
            self.sl1_long_a = rt(actual_entry * 0.985)
            self.sl1_long = max(self.sl1_long_a, self.sl1_long_b)
            self.sl2_long_a = rt(actual_entry * 0.985)
            self.sl2_long = max(self.sl2_long_a, self.sl2_long_b)
        else:
            self.e_s = actual_entry
            self.t_s = rt(actual_entry * 0.985)
            self.sl1_short_a = rt(actual_entry * 1.015)
            self.sl1_short = min(self.sl1_short_a, self.sl1_short_b)
            self.sl2_short_a = rt(actual_entry * 1.015)
            self.sl2_short = min(self.sl2_short_a, self.sl2_short_b)
