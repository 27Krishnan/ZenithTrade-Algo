"""
Gold Strategy — Telegram Alerts (Simplified)
"""
...
def send_morning_alert(levels_by_instrument: dict, live_states: dict | None = None):
    """Briefing with holding P/L."""
    from datetime import datetime
    import pytz
    IST  = pytz.timezone("Asia/Kolkata")
    date = datetime.now(IST).strftime("%d-%b-%Y")

    header = f"📅 <b>{date}</b> — ZenithTrade Algo — Gold Alert\n{'─'*25}"
    blocks = [header]
    ...
    for inst, lvl in levels_by_instrument.items():
        ...
        if l_st in ("ACTIVE_P1", "ACTIVE_P2"):
            ep = state.get("long_entry_price", 0)
            ltp = state.get("ltp", 0)
            pnl = round(ltp - ep, 2) if (ltp and ep) else 0
        ...
        if s_st in ("ACTIVE_P1", "ACTIVE_P2"):
            ep = state.get("short_entry_price", 0)
            ltp = state.get("ltp", 0)
            pnl = round(ep - ltp, 2) if (ltp and ep) else 0
...
def send_entry_triggered(instrument: str, direction: str, price: float, target: float, sl: float, symbol: str = ""):
    emoji = "🟢" if direction == "LONG" else "🔴"
    send(f"{emoji} <b>{instrument} {direction} Entry @ {_fmt(price)}</b>\nTgt: {_fmt(target)} | SL: {_fmt(sl)}", "tg_notify_entry")

def send_sl_hit(instrument: str, direction: str, sl_price: float, entry: float, pnl: float, lots: str = ""):
    send(f"🚨 <b>{instrument} {direction} SL HIT @ {_fmt(sl_price)}</b>\nPnL: <b>{'%+.2f' % pnl} pts</b>", "tg_notify_exit")

def send_lot1_target_hit(instrument: str, direction: str, target: float, lot1_pnl: float, sl2: float):
    send(f"🎯 <b>{instrument} {direction} Target @ {_fmt(target)}</b>\nPnL: <b>+{_fmt(lot1_pnl)} pts</b> | SL2: {_fmt(sl2)}", "tg_notify_exit")

def send_gap(instrument: str, direction: str, ltp: float, entry_level: float):
    gtype = "UP" if direction == "LONG" else "DOWN"
    send(f"⚠️ <b>{instrument} GAP {gtype} @ {_fmt(ltp)}</b>\nSkipped — Recalculating @ 9:15", "tg_notify_gap")

def send_gap_recovery(instrument: str, long_recovered: bool, short_recovered: bool, lvl: dict):
    msg = f"🔄 <b>{instrument} GAP RECOVERY</b>\n"
...
def send_rollover_warning(instrument: str, days_left: int, expiry: str):
    send(f"⏳ <b>{instrument} Rollover Intimation</b>\nExpiry in {days_left} working days ({expiry}).")

def send_sl2_updated(instrument: str, direction: str, old_sl: float, new_sl: float, window: str = ""):
    send(f"🔄 <b>{instrument} {direction} SL Trailed</b>\nNew SL: {_fmt(new_sl)}")

def send_sl2_locked(instrument: str, direction: str, current_sl: float):
    pass # No message needed if locked
